...
;Dh = 3000
  ...
  OPENR, 1, '/home/jess/RXJ2248/WSLAP/geo_delens_relens/img_position.dat'
  N_img=0
  readf, 1, N_img
  position=dblarr(4,N_img)
  readf, 1, position
  close, 1

z_src=dblarr(N_img)
FOR x=0,N_img-1 DO z_src(x)=position(3,x)
sys=intarr(N_img)
FOR x=0,N_img-1 DO sys(x)=position(0,x)

;  openr, 3, '/home/jess/RXJ2248/WSLAP/geo_delens_relens/f_k.dat'
;  N=0
;  readf, 3, N
;  f_k_get=dblarr(3,N)
;  readf,3, f_k_get
...
;  f_k(x)=f_k_get(1,WHERE(f_k_list(*) eq z_src(x))) 
;ENDFOR
  
  position(1,*)=position(1,*)-708.5
  position(2,*)=position(2,*)-708.5

  alphaX_512_pix = READFITS('/home/jess/RXJ2248/WSLAP/geo_delens_relens/recomp_alpha_x_pix.fits')
  alphaY_512_pix = READFITS('/home/jess/RXJ2248/WSLAP/geo_delens_relens/recomp_alpha_y_pix.fits')
  theta='/home/jess/RXJ2248/WSLAP/geo_delens_relens/theta.fits'
  src= '/home/jess/RXJ2248/WSLAP/geo_delens_relens/beta.fits'
  outfile = '/home/jess/RXJ2248/WSLAP/geo_delens_relens/ReImg.fits'
...
; = * = * = * = * = * = * = * = * = * = * = * = * = * = * = * 
  ...
  PRINT,'Delensing & Relensing ... '
    ...
    FOR j=0, N_img-1 DO BEGIN
	  scaled_alphaX(*,*) = alphaX(*,*)*f_k(1,j)
	  scaled_alphaY(*,*) = alphaY(*,*)*f_k(1,j)
	BetaX=0
	BetaY=0
